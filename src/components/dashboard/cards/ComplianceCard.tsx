...
import { T, TONE, cardStyle } from "@/lib/dashboardTheme";
...
const subCard: React.CSSProperties = { ...cardStyle, padding: 18, display: "flex", flexDirection: "column", flex: 1 };
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", paddingTop: 2 }}>
            {compItems.map((c) => (
              <div key={c.label} style={{
                ...
                overflow: "hidden", background: T.inset,
      ...
      <div style={subCard}>
        <LazyCard delay={800}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 11.5, color: T.muted }}>{integLogos.length} connected</span>
          </div>
          {/* Brand marks keep their own colours — they are identity, not data. */}
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", paddingTop: 2 }}>
            {integLogos.map((item) => (
              <div key={item.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 9.5, color: T.muted }}>{item.label}</span>
              </div>
            ))}
          </div>
        </LazyCard>
