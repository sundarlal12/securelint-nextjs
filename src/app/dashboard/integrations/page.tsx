...
function VcIcon() { return <svg viewBox="0 0 24 24" width="22" height="22"><path d="M24 22.525H0l12-21.05 12 21.05z" fill="#0a0a0a"/></svg>; }
...
export default function IntegrationsPage() {
  ...
  const pieData = [
    { name: "Connected", value: connected, c: "#16a34a" },
    { name: "Warning", value: warnings, c: "#d97706" },
    { name: "Disconnected", value: disconnected, c: "#dc2626" },
  ];

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 1400 }}>
      <div>
        <h2 style={{ fontSize: 24, fontWeight: 660, color: "#0a0a0a", letterSpacing: "-0.028em", margin: 0 }}>Integrations</h2>
        <p style={{ fontSize: 14, color: "#52525b", marginTop: 6 }}>Connect your tools and platforms to enable real-time secret detection and security monitoring.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {([
          { label: "Total Integrations", val: integrations.length, c: "#0d9488", d: "M13 10V3L4 14h7v7l9-11h-7z" },
          { label: "Connected", val: connected, c: "#16a34a", d: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
          { label: "Warnings", val: warnings, c: "#d97706", d: "M12 2L3 20h18L12 2zM12 9v4m0 4h.01" },
          { label: "Disconnected", val: disconnected, c: "#dc2626", d: "M18.36 5.64l-12.72 12.72M5.64 5.64l12.72 12.72" },
        ] as const).map((s, i) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div style={{ ...cs, padding: "16px 20px" }}>
          <LazyCard delay={600}>
            <div style={{ fontSize: 10, color: "#52525b", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Integration Activity (7d)</div>
            <ResponsiveContainer width="100%" height={80}>
              <BarChart data={activityData} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                <XAxis dataKey="d" tick={{ fill: "#71717a", fontSize: 9 }} axisLine={false} tickLine={false} />
                <Bar dataKey="v" fill="#0d9488" radius={[3, 3, 0, 0]} barSize={16} />
              </BarChart>
            </ResponsiveContainer>
          </LazyCard>
        </div>
        <div style={{ ...cs, padding: "16px 20px" }}>
          <LazyCard delay={700}>
            <div style={{ fontSize: 10, color: "#52525b", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Status Overview</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {pieData.map((p, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, color: "#0a0a0a", fontWeight: 600 }}>{p.value}</span>
                    <span style={{ fontSize: 11, color: "#52525b" }}>{p.name}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {categories.map(cat => (
          <button key={cat} onClick={() => setFilter(cat)}
            style={{ padding: "6px 14px", fontSize: 12, fontWeight: 600, borderRadius: 8, cursor: "pointer", transition: "all .15s",
              border: filter === cat ? "1px solid rgba(22,163,74,0.14)" : "1px solid #e9e9ec",
              background: filter === cat ? "#bbf7d0" : "#ffffff",
              color: filter === cat ? "#16a34a" : "#52525b",
            }}>{cat}</button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {visible.map((integ, idx) => {
          ...
          const stColor = integ.status === "Connected" ? "#16a34a" : integ.status === "Warning" ? "#d97706" : "#dc2626";
          const stBg = integ.status === "Connected" ? "#f0fdf4" : integ.status === "Warning" ? "#faf8f0" : "#f8f3f3";
          return (
            <LazyCard key={integ.id} delay={300 + idx * 80}>
              <div id={`integ-${integ.id}`}
                style={{ ...cs, padding: "20px", display: "flex", flexDirection: "column", gap: 14, transition: "border-color .2s",
                  borderColor: expanded ? "rgba(22,163,74,0.14)" : "#e9e9ec",
                  boxShadow: expanded ? "0 0 0 2px rgba(22,163,74,0.14)" : "none",
                }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 42, height: 42, borderRadius: 12, background: "#f4f4f5", border: "1px solid #e9e9ec", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><integ.Icon /></div>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0a0a0a" }}>{integ.name}</div>
                      <div style={{ fontSize: 10, color: "#52525b", marginTop: 1 }}>{integ.cat}</div>
                    </div>
                  </div>
                ...
                <p style={{ fontSize: 12, color: "#52525b", lineHeight: 1.6, margin: 0 }}>{integ.desc}</p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, padding: "12px", borderRadius: 10, background: "#f4f4f5", border: "1px solid #e9e9ec" }}>
                  {integ.stats.map((s, i) => (
                    <div key={i} style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0d9488" }}>{s.v}</div>
                      <div style={{ fontSize: 9, color: "#52525b", marginTop: 2 }}>{s.l}</div>
                    </div>
                  ))}
                </div>
                {expanded && (
                  <div style={{ padding: "12px", borderRadius: 10, background: "#f4f4f5", border: "1px solid #e9e9ec" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#52525b", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Configuration</div>
                    <div style={{ fontFamily: "monospace", fontSize: 11, color: "#0a0a0a", wordBreak: "break-all", marginBottom: 4 }}>{integ.cfg.wh}</div>
                    <div style={{ fontSize: 10, color: "#52525b" }}>Scope: {integ.cfg.scope}</div>
                  </div>
                )}
                <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
                  <button onClick={() => setSelected(expanded ? null : integ.id)}
                    style={{ flex: 1, padding: "9px 14px", fontSize: 11, fontWeight: 700, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all .15s",
                      border: integ.status === "Connected" ? "1px solid #e9e9ec" : "none",
                      background: integ.status === "Connected" ? "#f4f4f5" : "#0a0a0a",
                      color: integ.status === "Connected" ? "#52525b" : "#ffffff",
                    }}>
                    {integ.status === "Connected" ? (expanded ? "Hide config" : "Configure") : "Connect"}
                  </button>
                  <button style={{ width: 36, height: 36, borderRadius: 8, border: "1px solid #e9e9ec", background: "#f4f4f5", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#52525b", flexShrink: 0 }}>
