...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) return { title: "Article Not Found — SecureLint Blog" };
  return {
    title: `${post.title} — SecureLint Blog`,
    description: post.excerpt,
    alternates: { canonical: `https://securelint.in/blog/${post.slug}` },
    keywords: [
      "SecureLint", "browser extension security", "API key masking",
      "phishing detection", "credential leak prevention",
    ...
    openGraph: {
      type: "article",
      url: `https://securelint.in/blog/${post.slug}`,
...
export default function ArticlePage({ params }: Props) {
  ...
  /* Only the Meeting Mode article has full content; others get a "coming soon" shell */
  const hasFull = params.slug === "meeting-mode-blur-secrets-during-video-calls";
  ...
  const articleSchema = {
    ...
    publisher: { "@type": "Organization", name: "SecureLint by VAPTLabs",
      logo: { "@type": "ImageObject", url: "https://securelint.in/icons/icon-128.png" } },
    mainEntityOfPage: { "@type": "WebPage", "@id": `https://securelint.in/blog/${post.slug}` },
  };
  const breadcrumbSchema = {
    "@context": "https://schema.org", "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home",
        item: { "@type": "WebPage", "@id": "https://securelint.in" } },
      { "@type": "ListItem", position: 2, name: "Blog",
        item: { "@type": "WebPage", "@id": "https://securelint.in/blog" } },
      { "@type": "ListItem", position: 3, name: post.title,
        item: { "@type": "WebPage", "@id": `https://securelint.in/blog/${post.slug}` } },
    ],
  };
  ...
  return (
    <>
      ...
      {hasFull && <MeetingModeFaqSchema />}

      <SiteHeader />
      <div className={s.page}>
        {hasFull ? (
          <MeetingModeArticle post={post} />
        ) : (
...
/* ─── MeetingModeFaqSchema ────────────────────────────────── */
function MeetingModeFaqSchema() {
  const faqSchema = {
    "@context": "https://schema.org", "@type": "FAQPage",
    mainEntity: [
      {
        "@type": "Question",
        name: "Which video call apps does SecureLint Meeting Mode detect?",
        acceptedAnswer: { "@type": "Answer", text: "SecureLint Meeting Mode detects Zoom Web Client, Google Meet (meet.google.com), Microsoft Teams (teams.microsoft.com), Webex, Jitsi Meet, and any site that requests camera or microphone permissions. When any of these are open in a tab, secrets across all your other tabs are immediately blurred." },
      },
      {
        "@type": "Question",
        name: "What types of secrets does SecureLint blur in Meeting Mode?",
        acceptedAnswer: { "@type": "Answer", text: "SecureLint blurs AWS access keys and secret keys, GitHub personal access tokens, Stripe secret and restricted keys, OpenAI API keys, Razorpay keys, Jira API tokens, database connection strings, JWT tokens, SSH private keys, and 100+ other credential patterns — all in real time without any server communication." },
      },
      {
        "@type": "Question",
        name: "Does SecureLint record my screen or send data anywhere?",
        acceptedAnswer: { "@type": "Answer", text: "No. SecureLint runs entirely on-device using a local regex engine inside the browser extension. Nothing from your screen, tabs, or credentials is ever captured, uploaded, or sent to any server. Detection happens locally and the blur is applied locally." },
      },
      {
        "@type": "Question",
        name: "Does the blur automatically lift when my meeting ends?",
        acceptedAnswer: { "@type": "Answer", text: "Yes. SecureLint continuously watches for the meeting tab to close or for camera/microphone permissions to be revoked. The moment the call ends, all blur overlays are removed and your browser tabs return to normal visibility automatically." },
      },
      {
        "@type": "Question",
        name: "Can I override Meeting Mode during a call?",
        acceptedAnswer: { "@type": "Answer", text: "Yes. You can click the SecureLint extension icon and temporarily pause Meeting Mode for a specific tab or globally. The override lasts until the end of the call or until you re-enable it manually. A visual indicator in the toolbar reminds you that secrets are currently exposed." },
      },
    ],
  };
...
/* ─── MeetingModeArticle ─────────────────────────────────── */
function MeetingModeArticle({ post }: { post: typeof POSTS[number] }) {
  const toc = [
  ...
  return (
    <div className={s.outer}>
      ...
      <article className={s.article}>
        ...
        {/* Cover banner */}
        <div className={s.coverBanner} style={{ background: post.gradient }} aria-hidden="true">
          <span className={s.coverBannerDeco1} style={{ background: post.accentColor }}/>
          <span className={s.coverBannerDeco2} style={{ background: post.accentColor }}/>
          <p className={s.coverBannerText}>
            Meeting Mode —<br/>
            <span style={{ color: post.accentColor }}>Blur secrets. Automatically.</span>
          </p>
        </div>
        ...
        <div className={s.prose}>
          ...
          <p>
            ...
            It happens in seconds and you don&apos;t notice until someone asks, <em>&ldquo;is that your
            production key?&rdquo;</em>
          </p>
          <p>
          ...
          <div className={s.note}>
            <strong>Privacy note:</strong> No audio, video, or screen content is ever read, captured,
            or transmitted by SecureLint. The extension only reads tab metadata (URL, title) and
            permission state — both are local browser signals.
          </div>
          ...
          <p>
            ...
            major cloud and SaaS platform. The categories include:
          </p>
          <ul>
            <li><strong>Cloud provider keys</strong> — AWS Access Keys (<code>AKIA…</code>), AWS Secret Access Keys, GCP service account JSON tokens, Azure client secrets</li>
            ...
            <li><strong>Auth tokens</strong> — JWT tokens (three dot-separated base64 segments), OAuth access tokens and refresh tokens</li>
          ...
          <p>
            ...
            page as well as input field values that are not of type <code>password</code> (since
            those are already hidden by the browser). The match runs against the DOM, not raw HTTP
            responses, so it works on single-page apps, dashboards loaded via AJAX, and
            server-side-rendered pages alike.
          </p>
          ...
          <ol>
            ...
            <li>
              ...
              content added to the DOM (lazy-loaded panels, WebSocket pushes, etc.) and re-runs the
          ...
          <p>Meeting Mode requires no configuration. It is enabled by default after you install the SecureLint Chrome extension. Here is the full setup path:</p>
          <div className={s.checklist}>
            <ul>
              ...
              <li><span className={s.checkIcon}>✅</span><span>Open a Google Meet or Zoom tab. You will see a green indicator in the SecureLint toolbar icon showing the call was detected.</span></li>
              <li><span className={s.checkIcon}>✅</span><span>Switch to any tab containing API keys or credentials. Sensitive values will be overlaid with blur masks.</span></li>
          ...
          <p>
            ...
            policy via the SecureLint admin console, preventing employees from accidentally disabling
            it before a call.
          </p>
          ...
          <ul>
            ...
            <li>The meeting tab navigates away from the call URL (e.g., you close the video room and land on the Google Meet home screen).</li>
            <li>The page that held the camera/microphone permission is unloaded or the permissions are revoked.</li>
            <li>You manually click <strong>Pause Meeting Mode</strong> in the SecureLint panel (the blur lifts for 10 minutes, after which it re-arms automatically).</li>
          </ul>
          <p>
          ...
          <p>
            ...
            A security engineer showing a client a scan report needs the credentials tab right there.
          </p>
          <p>
          ...
          <ol>
            <li>
              <strong>The unexpected call</strong> — Someone shares a Slack Huddle link mid-conversation
              and you join before you have time to sweep your tabs.
            </li>
            <li>
              <strong>New content after the call starts</strong> — A CI/CD system pushes a deployment
              key to a dashboard tab that was already open and &ldquo;safe&rdquo; when you started sharing.
            </li>
          </ol>
          <p>
            Meeting Mode handles both. It is always watching, it activates instantly, and it covers
            dynamic content that appears after the call begins. You keep every tab open that you need,
            and SecureLint makes sure none of the credentials in them reach the screen share.
          </p>
        </div>
        ...
        <section className={s.faqSection} aria-labelledby="faq">
          ...
          <div className={s.faqItem}>
            ...
            <p className={s.faqA}>SecureLint blurs AWS access and secret keys, GitHub personal access tokens, Stripe live secret keys, OpenAI project keys, Razorpay keys, Jira API tokens, database connection strings, JWT tokens, SSH private key blocks, and 100+ other credential patterns — all identified in real time locally in the browser, with no server communication.</p>
          </div>

          <div className={s.faqItem}>
            <p className={s.faqQ}>Does SecureLint record my screen or send data anywhere?</p>
            <p className={s.faqA}>No. SecureLint runs entirely on-device using a local regex engine inside the browser extension. Nothing from your screen, open tabs, or credentials is ever captured, uploaded, or transmitted to any server. Detection and blur both happen locally.</p>
          </div>
          ...
          <div className={s.faqItem}>
            ...
            <p className={s.faqA}>Yes. You can click the SecureLint extension icon and choose Pause Meeting Mode for 10 minutes. A visible warning badge in the toolbar reminds you that secrets are temporarily exposed. Meeting Mode re-arms automatically when the 10-minute window expires or when a new meeting is detected.</p>
          </div>
        </section>
...
/* ─── ComingSoon fallback ─────────────────────────────────── */
function ComingSoon({ post }: { post: typeof POSTS[number] }) {
  return (
    <div className={s.comingSoon}>
      <p style={{ color: "#6b7280", fontSize: 13, marginBottom: 16 }}>Blog post coming soon</p>
      <h1 className={s.comingSoonTitle}>{post.title}</h1>
      <p className={s.comingSoonDesc}>{post.excerpt}</p>
      <Link href="/blog" className={s.comingSoonBack}>← Back to all articles</Link>
    </div>
  );
}
